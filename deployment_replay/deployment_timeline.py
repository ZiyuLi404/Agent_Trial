...
# Allow running directly (`python deployment_replay/deployment_timeline.py`) as
# well as via `python -m deployment_replay.deployment_timeline`: put repo root on
# sys.path so `trial.*`, `deployment_replay.*` and `AgentClinic.*` all resolve.
sys.path.insert(0, str(Path(__file__).resolve().parents[1]))

from trial.version_manager import open_new_version
from trial.trial_manager import stream_cases, run_case
from deployment_replay.replay_evaluator import run_replay_case
from deployment_replay.oracle_evaluator import run_oracle_case
from trial.logger import log_deployment_case
