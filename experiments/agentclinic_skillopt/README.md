...
```bash
python -m change_generators.skills.optimizers.skillopt_lite.evaluator \
  --skill change_generators/skills/artifacts/diagnostic_reasoning/v000.md \
  --harness change_generators/harnesses/artifacts/agentclinic/diagnostic_efficiency/v000.toml \
  --dataset MedQA \
  --split train \
  --eval_limit 2 \
  --limit 2 \
...
```text
results/skillopt_workspaces/agentclinic/<doctor-model>/
├── skill.md
└── .skillopt/
    ├── samples/
    │   ├── failed/*.md
