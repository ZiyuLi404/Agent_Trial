...
class DeploymentTimeline:
    ...
    def _run_epoch(self, epoch_idx, treatment_model, n_cases):
        ...
        for case_id, timestamp, scenario, _arm in stream_cases(
            ...
            # ── Control run ──────────────────────────────────────────────────
            ctrl_dx, ctrl_correct, ctrl_consult = run_case(scenario, control_config)
            ctrl_record = {
                "case_id": case_id,
            ...
            # ── Treatment run ────────────────────────────────────────────────
            tmt_dx, tmt_correct, tmt_consult = run_case(scenario, treatment_config)
            tmt_record = {
                "case_id": case_id,
                "paired_case_id": case_id,
                "dataset": self.dataset,
        ...
        if epoch_idx > 0 and self.past_records:
            ...
            for past_rec in self.past_records:
                ...
                replay_dx, replay_correct = run_replay_case(
                ...
                oracle_dx, oracle_correct, oracle_transcript = run_oracle_case(
                    dataset=self.dataset,
            ...
            print(f"  Control accuracy:    {control_acc:.4f}  ({control_correct}/{total_n})")
